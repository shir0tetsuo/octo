...
function buildCell(entity) {
    ...
    applyDataStateChannel(cell, entity.aesthetics?.bar)
    ...
    owner.textContent = (entity.ownership ?? "00000000").split('-')[0];
...
function buildURL(x, y, z, redirect=null) {
    const params = new URLSearchParams({
        x: x,
        y: y,
        z: z
...
function updateNavLinks(x, y, z) {
    // Get the current redirect parameter to preserve it during navigation
    const currentParams = new URLSearchParams(window.location.search);
    const currentRedirect = currentParams.get("redirect");
    
    $("#x_plus").attr("href", buildURL(x + 1, y, z, currentRedirect));
    $("#x_minus").attr("href", buildURL(Math.max(0, x - 1), y, z, currentRedirect));

    $("#y_plus").attr("href", buildURL(x, y + 1, z, currentRedirect));
    $("#y_minus").attr("href", buildURL(x, Math.max(0, y - 1), z, currentRedirect));
}
...
function ChangeAreaNav() {
    ...
    redirect = encodeURIComponent(window.location.pathname + window.location.search);
    nav.href = 'area.html?redirect=' + redirect; // This will point to the user page
}
